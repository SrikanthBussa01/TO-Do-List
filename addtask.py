...
class AddTaskPage:
    def __init__(self, root, user, databaseConnection):
        ...
        self.wel_label = tk.Label(self.frame1, text="Create Task",font = font.Font(size=16, weight="bold"))
        ...
        self.select_status = tk.StringVar(value = "None")
        ...
        self.select_priority = tk.StringVar(value = "None")
        ...
        self.blog = tk.Button(self.f3, text="Create Task", command=self.add, font = font.Font(size = 10,weight="bold"))
        self.blog.pack(side=tk.LEFT, padx = 15) 
    ...
    def add(self):
        ...
        if(len(title) == 0 or len(desc) == 0 or len(dat)==0 or status=="None" or priority == "None"):
            messagebox.showerror("Add Task Error", "All the Fields in Add Task Page should not be empty. Please fill up all fields.")
        elif(len(title)>50):
            messagebox.showerror("Add Task Error", "Title cannot be more than 50 characters.")
        elif(len(desc)>200):
            messagebox.showerror("Add Task Error", "Description cannot be more than 200 characters.")
        else:
            try:
                dat_valid = datetime.strptime(dat,'%Y-%m-%d' )
                query = "insert into task(Title,Description,Status,Priority,Date,UserID) values (%s, %s, %s, %s, %s, %s)"
                data = (title,desc,status,priority,dat,userid)
                self.databaseConnection.execute_query(query,data)
                messagebox.showinfo("Add Task Status", "Task Added successfully. Redirecting to Task Management Page to create or manage tasks...")
                self.root.destroy()
                taskmgmt_page = tk.Tk()
                TaskManagementPage(taskmgmt_page, self.username, self.databaseConnection)
                taskmgmt_page.mainloop()
            except ValueError:
                messagebox.showerror("Add Task Error", "Date should be of yyyy/mm/dd format. Please input correct date")
