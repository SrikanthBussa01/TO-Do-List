...
class EditTaskPage:
    def __init__(self, root, user, databaseConnection):
        ...
        self.wel_label = tk.Label(self.frame1, text="Edit Task",font = font.Font(size=16, weight="bold"))
        self.wel_label.pack(pady=15)
        self.wel_label = tk.Label(self.frame1, text="Select Task to Edit", font = font.Font(size = 10,weight="bold"))
        ...
        self.blog = tk.Button(self.f, text="Edit Selected Task", command=self.edit, font = font.Font(size = 10,weight="bold"))
        self.blog.pack(side=tk.LEFT, padx = 15) 
    ...
    def edit(self):
        from edittaskdetails import EditPage
        tid = self.select_task.get()
        if(tid>0):
            response = messagebox.askyesno("Edit Task", "Are you sure you want to edit the task?")
            if response:
                self.root.destroy()
                edit_page = tk.Tk()
                EditPage(edit_page, self.username, tid, self.databaseConnection)
                edit_page.mainloop()
            else:
                messagebox.showinfo("Edit Task Status", "You've cancelled editing the task. Redirecting to Edit task page to select correct task to edit")
                self.root.destroy()
                edittask_page = tk.Tk()
                EditTaskPage(edittask_page, self.username, self.databaseConnection)
                edittask_page.mainloop()
        else:
            messagebox.showerror("Edit Task Error", "You've not selected any task to edit. Select task to edit")
